...
app.put('/tutorials/:id',async (req,res)=>{
	 ...
	 //  console.log(req.params.id);console.log(req.body);
	    const { _id, ...datas} = req.body;
		//console.log(datas);
	  // return;
	  let data;
	  try{
        data =  await Tutorial.findOneAndUpdate({_id:req.params.id},datas,{new:true});
	  }
	  catch(error)
	  {
		  console.log(error);
...
app.delete('/tutorials/:id',async (req,res)=>{
   ...
   let data;
	  try{
        data =  await Tutorial.findOneAndRemove({_id:req.params.id});
	  }
	  catch(error)
	  {
		  console.log(error);
...
app.delete('/updatetodo/:id',async (req,res)=>{
   const removedTodo = await Todo.findOneAndRemove({_id:req.params.id})
   res.status(200).json({message:removedTodo})
})
